...
def extract_filter_values(values):
    if not isinstance(values, (list, str)):
        raise ValueError("Filter values must be list of strings or a string")
    if isinstance(values, str):
        values = [values]
    else:
        for value in values:
            if not isinstance(value, str):
                raise ValueError("Filter value must be string")
    return values


class _FilterExactMatch:
    def __init__(self, prop_name, values, inverse=False):
        self.prop_name = prop_name
        self.values = extract_filter_values(values)
        self.inverse = inverse

    def is_matched(self, builder_name, props):
        prop_value = props.get(self.prop_name, None)
        if prop_value is None:
            return False
        found = prop_value in self.values
        return not found if self.inverse else found


class _FilterRegex:
    def __init__(self, prop_name, regexes):
        self.prop_name = prop_name
        self.regexes = [re.compile(regex) for regex in extract_filter_values(regexes)]
    ...
    def is_matched(self, builder_name, props):
        ...
        for regex in self.regexes:
            if regex.match(prop_value) is not None:
                return True
        return False
...
class _FilterCallable:
    def __init__(self, c):
        if not callable(c):
            raise ValueError("The 'callable' filter is not a callable function")
        self.callable = c

    def is_matched(self, builder_name, props):
        return self.callable(builder_name, props)


class _FilterCombined:
    def __init__(self):
        self.filters = []

    def add_subfilter(self, filter):
        self.filters.append(filter)

    def is_matched(self, builder_name, props):
        for filter in self.filters:
            if not filter.is_matched(builder_name, props):
                return False
        return True
...
class _OldBuildFilterSet:
    ...
    def is_matched(self, builder_name, props):
        ...
        for filter in filters:
            if filter.is_matched(builder_name, props):
                return True
        return False
...
class _OldBuildTracker:
    ...
    def _on_new_cancellable(self, id_tuple, builder_name, sourcestamps):
        ...
        for ss in sourcestamps:
            ...
            # Note that it's enough to match build by a single branch.
...
class OldBuildCanceller(BuildbotService):
    ...
    def reconfigService(self, name, filters):
        ...
        filter_set_object = OldBuildCanceller.filter_dicts_to_filter_set_object(filters)
    ...
    @classmethod
    def build_filter_for_dict(cls, filter_dict):
        result_filter = _FilterCombined()

        all_keys = ['builders', 'callable']
        callable = filter_dict.get('callable', None)
        if callable is not None:
            result_filter.add_subfilter(_FilterCallable(callable))

        suffix_to_builder_list = [
            ('_eq', lambda prop, values: _FilterExactMatch(prop, values, inverse=False)),
            ('_not_eq', lambda prop, values: _FilterExactMatch(prop, values, inverse=True)),
            ('_regex', _FilterRegex),
            ('_not_regex', _FilterRegexInverse),
        ]

        for prop_name in ('project', 'codebase', 'repository', 'branch'):
            for suffix, filter_factory in suffix_to_builder_list:
                all_keys.append(prop_name + suffix)
                values = filter_dict.get(prop_name + suffix, None)
                if values is not None:
                    try:
                        result_filter.add_subfilter(filter_factory(prop_name, values))
                    except Exception as e:
                        config.error('{}: When processing filters property {}: {}'.format(
                            cls.__name__, prop_name + suffix, str(e)))

        for key in filter_dict.keys():
            if key not in all_keys:
                config.error('{}: Unknown filter key {}'.format(cls.__name__, key))
    ...
    def check_filters(cls, filters):
        if not isinstance(filters, list):
            config.error('{}: The filters argument must be a list of dictionaries'.format(
                cls.__name__))
        ...
        for filter in filters:
            ...
            if 'builders' not in filter:
                config.error('{}: The filters must include builders field'.format(
                    cls.__name__))
            try:
                extract_filter_values(filter['builders'])
            except Exception as e:
                config.error('{}: When processing filters property builders: {}'.format(
                    cls.__name__, str(e)))

            cls.build_filter_for_dict(filter)

    @classmethod
    def filter_dicts_to_filter_set_object(cls, filters):
        filter_set = _OldBuildFilterSet()

        for filter in filters:
            filter_set.add_filter(extract_filter_values(filter['builders']),
                                  cls.build_filter_for_dict(filter))
